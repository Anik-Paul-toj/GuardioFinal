...
contract TouristID is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
     ...
     * @param tokenMetadataURI Metadata URI for the NFT
     */
    function mintTouristID(
        ...
        require(userToTokenId[to] == 0 && balanceOf(to) == 0, "User already has a Tourist ID");
        ...
        userToTokenId[to] = tokenId + 1; // Store as tokenId + 1 to avoid conflict with default 0
    ...
    function getUserTokenId(address user) public view returns (uint256) {
        uint256 storedValue = userToTokenId[user];
